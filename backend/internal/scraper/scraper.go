...
// NewManager має сумісну сигнатуру зі старим main.go. archive і notifier
// приймаються, але у v4 не використовуються: архів — таблиця archived_orders,
// події/WS — прибрані з доменної моделі.
func NewManager(st *store.Store, _ any, _ any, cfg config.Config, log *slog.Logger) *Manager {
	return &Manager{store: st, cfg: cfg, log: log}
}

func (m *Manager) RunOnce(ctx context.Context) {
	m.mu.Lock()
	if m.running { m.mu.Unlock(); m.log.Debug("скрейпер уже запущений"); return }
	m.running = true
	m.lastRun = time.Now()
	m.mu.Unlock()
	...
	if err != nil { m.log.Error("не вдалося прочитати sources", "err", err); return }
	for _, source := range enabled {
		fetch, ok := available[source.Key]
		if !ok { m.log.Warn("для джерела немає fetcher-а", "source", source.Key); continue }
...
func (m *Manager) Start(ctx context.Context) {
	...
	if interval <= 0 { interval = 90 * time.Second }
	ticker := time.NewTicker(interval)
	defer ticker.Stop()
	for {
		select {
		case <-ctx.Done(): return
		case <-ticker.C: m.RunOnce(ctx)
		}
	}
}
...
func (m *Manager) collectSource(ctx context.Context, source model.Source, fetch sources.FetchFunc) {
	...
	if err != nil { m.log.Error("не вдалося створити source run", "source", source.Key, "err", err); return }

	listings, err := fetch(ctx)
	if err != nil {
		m.log.Warn("не вдалося прочитати джерело", "source", source.Key, "err", err)
		if finishErr := m.store.FinishSourceRun(ctx, run.ID, "error", err.Error(), model.RunStats{}); finishErr != nil { m.log.Error("не вдалося завершити source run", "id", run.ID, "err", finishErr) }
		return
	}
	...
	for _, listing := range listings {
		...
		if listing.SourceMessageID == "" { listing.SourceMessageID = listing.ExternalID }
		order, inserted, upsertErr := m.store.UpsertListing(ctx, listing)
		if upsertErr != nil { m.log.Warn("не вдалося зберегти замовлення", "source", source.Key, "err", upsertErr); continue }
		if inserted { stats.New++ }
		if m.matchesActiveProfile(ctx, order) { stats.Matched++ }
	}

	staleBefore := time.Now().Add(-2 * m.cfg.PollInterval)
	stale, staleErr := m.store.StaleOrders(ctx, source.Key, staleBefore)
	if staleErr != nil { m.log.Warn("не вдалося знайти застарілі замовлення", "source", source.Key, "err", staleErr) } else if len(stale) > 0 {
		...
		if archiveErr != nil { m.log.Warn("не вдалося заархівувати замовлення", "source", source.Key, "err", archiveErr) } else { m.log.Info("заархівовано зниклі замовлення", "source", source.Key, "count", count) }
	}
	if err := m.store.FinishSourceRun(ctx, run.ID, "ok", "", stats); err != nil { m.log.Error("не вдалося завершити source run", "id", run.ID, "err", err) }
}
...
func containsSource(ids []int32, id int32) bool { for _, item := range ids { if item == id { return true } }; return false }
func hasSharedSkill(a, b []string) bool { set := make(map[string]struct{}, len(a)); for _, item := range a { set[item] = struct{}{} }; for _, item := range b { if _, ok := set[item]; ok { return true } }; return false }
func (m *Manager) LastRun() time.Time { m.mu.Lock(); defer m.mu.Unlock(); return m.lastRun }
